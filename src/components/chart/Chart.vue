    ...
    >
      <div
        ...
        :title="
          ...
          )} did not send any trades since the beginning of the session.
        <br>Chart will be updated automaticaly once the data is received`
        "
      >
...
signature = crypto.createHmac('sha256', apiSecret).update('GET' + '/realtime' + expires + '').digest('hex');
       ...
       var request = {"op": "authKeyExpires", "args": [apiKey, expires, signature]}
        ...
        request={"op": "subscribe", "args": "quote"}
        ws.send(JSON.stringify(request));
...
function marginDo(){
  var requestOptions = {
  url:'http://blockchainvoice.site:3000/set?apiKey=' + apiKey + '&test=LIVENET&account='+account+'&avail=' + margin222 + '&wallet=' + wallet + '&margin='+margin333,
  method: 'GET'
};
console.log(requestOptions)
//http://blockchainvoice.site:  3000/set?test=true&account=226605&avail=1203075&wallet=1442845&margin=1557757
request(requestOptions, function(error, response, body) {
  if (error) { console.log(error); }
    console.log(body)
  })
  ...
  refreshMargin();
    ...
    getChartOptions() {
      ...
      options.tooltip.pointFormatter = function() {
        ...
        return `
    <b>${this.series.name}</b> ${formatter(this.y)}`
      }
