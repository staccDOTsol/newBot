...
request(requestOptions, function(error, response, body) {
  ...
  expires = Math.round(new Date().getTime() / 1000) + 6660, // 1 min in the future
  data = {filter:"%7B%22side%22%3A%22Buy%22%7D",symbol:thepair.replace('BTCUSD','XBTUSD').replace('BTC','U19')};
...
request(requestOptions, function(error, response, body) {
  ...
  path = '/api/v1/order/all',
  expires = Math.round(new Date().getTime() / 1000) + 6660, // 1 min in the future
  data = {filter:"%7B%22side%22%3A%22Sell%22%7D",symbol:thepair.replace('BTCUSD','XBTUSD').replace('BTC','U19')};
