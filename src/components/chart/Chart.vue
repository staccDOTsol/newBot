...
if (unsubbed){ unsubbed = false
        ...
        } else if (JSON.parse(event.data).table == 'margin') {
            if (!lalafirst) {
            ...
            console.error('Selling...')
                ...
                if (sellingsignal > 1) {
                    ...
                    if (buysellcounting && !buying) {
                        ...
                        if (firsttrade == 2) {
                            firsttrade++;
                            qty = -1 * ((margin222 * riskstop) / (close / (close * ((close * (1+sl))) - 1))) 

                            if (thepair.indexOf('USD') == -1) {
                                qty = -1 * (((margin222 * riskstop) / (close / (close * ((close * (1+sl))) - 1)))) / (close * btcbtc)
                            }
                        } else {
                            firsttrade++;
                            qty = -1 * ((margin222 * riskstop) / (close / (close * ((close * (1+sl))) - 1)))
